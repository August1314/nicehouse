...
- [ ] **阈值判断**：判断PM2.5、温度、电力消耗等是否超过设定阈值
- [ ] **自动联动**：超标时自动开启新风系统、净化器、空调，或关闭空调节能
...
- **配置字段：**
  ...
  - `energyConsumptionThreshold` - 电力消耗过高阈值（默认10 kWh/天）
...
namespace NiceHouse.EnvironmentControl
{
    ...
    public class EnvironmentThresholds : ScriptableObject
    {
        ...
        [Header("电力消耗阈值")]
        public float energyConsumptionThreshold = 10f;  // 电力消耗过高阈值（kWh/天）
    }
}
```
