...
- **手电筒开关控制**
  - 鼠标左键点击手电筒，开关灯光。
  - 按住右键进入“聚光模式”，光束变窄、亮度增强。
- **电量消耗与提示**
  - 手电筒开启时，电量逐渐减少。
  - 电量低于 10% 时，UI 提示“电量不足”。
- **充电逻辑**
  - 将手电筒放置在充电底座上，自动充电。

**对应代码建议：**

- `FlashLightController.cs`：管理手电筒的开关、聚光模式、电量消耗与充电逻辑。
- `FlashLightUI.cs`：负责 UI 提示与电量显示。

**与模型相关的分割与交互建议：**

- **手电筒（FlashLight）**
  - 分割要求：
    - 手电筒主体与光源分离，光源作为子物体，挂载 Light 组件。
  - 交互目的：
    - 鼠标左键控制开关，右键控制聚光模式。
    - 电量低时，手电筒光源闪烁提示。
  - 所需组件：
    - 父物体：`DeviceDefinition` + `FlashLightController` + `BoxCollider`
    - 子物体：`Light`（Spot 类型）
